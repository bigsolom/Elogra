...
public class Search {
	public Result goSearch(String srcID, String destID){
		...
		String sql = "select * from " + DBConstants.LIVE_FARES + " where " + DBConstants.LIVE_FARES_SRC_ID + " = " + srcID + " and " + DBConstants.LIVE_FARES_DEST_ID + " = " + destID;
		DBConnection.QueryResult qr = dbc.executeQuery(sql);
		
		res.setLiveFare(qr.getObject(0, DBConstants.LIVE_FARES_FARE).toString());
		
		sql = "select * from " + DBConstants.HISTORY_FARES + " where " + DBConstants.HISTORY_FARES_SRC_ID + " = " + srcID + " and " + DBConstants.HISTORY_FARES_DEST_ID + " = " + destID;
		...
		sql = "select * from " + DBConstants.ENTRIES + " where " + DBConstants.ENTRIES_SRC_ID + " = " + srcID + " and " + DBConstants.ENTRIES_DEST_ID + " = " + destID;
		qr = dbc.executeQuery(sql);
